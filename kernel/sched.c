...
static union task_union init_task = {INIT_TASK,};   // 定义初始任务的数据	// tsz: #personal 共用体使用内存量的占空间最大的成员的量
...
struct {
	long * a;
	short b;
	} stack_start = { & user_stack [PAGE_SIZE>>2] , 0x10 }; // tsz: #book 指向一个user_stack的最后一项，前32位赋值给了sp，后16位是段选择子给了ss，是内核数据段
/*
...
void sched_init(void)	//tsz: #course scheduling init，即进程调度的初始化
{
	...
	for(i=1;i<NR_TASKS;i++) {	//tsz: #course #book task[0]已经被手动设置，这里清空了还没用的task中的元素和gdt空间
		task[i] = NULL;	//tsz: #course task_struct的指针，拿这个去调度进程，64个元素，高版本不限定数量
			// tsz: #course task_struct放在task_union中，所以task中的指针指向了task_union的一部分
		p->a=p->b=0;	// tsz: #personal ab定义见head.h
		p++;
		p->a=p->b=0;
		p++;
	}
/* Clear NT, so that we won't have troubles with that later on */
    ...
    // 定时计数值。
	//tsz: #course 继续trap_init中没有做完的工作，设定中断；#personal 下面几行对8253定时器进行设置
	outb_p(0x36,0x43);		/* binary, mode 3, LSB/MSB, ch 0 */
	outb_p(LATCH & 0xff , 0x40);	/* LSB */
	outb(LATCH >> 8 , 0x40);	/* MSB */	// tsz: #book 10ms一次
    // 设置时钟中断处理程序句柄(设置时钟中断门)。修改中断控制器屏蔽码，允许时钟中断。
    // 然后设置系统调用中断门。这两个设置中断描述符表IDT中描述符在宏定义在文件
    // include/asm/system.h中。
	set_intr_gate(0x20,&timer_interrupt);	//tsz: #course #personal 中断,在system_call.s中;因为时钟中断与进程调度密切相关，一次被放在sched.c中
	outb(inb_p(0x21)&~0x01,0x21);	// tsz: #book 将8253屏蔽码打开
	set_system_gate(0x80,&system_call);	//tsz: #course 系统调用
}
