...
void sched_init(void)	//tsz: #course scheduling init，即进程调度的初始化
{
    ...
    // 也即gdt数组第4项的地址。
	set_tss_desc(gdt+FIRST_TSS_ENTRY,&(init_task.task.tss));	//tsz: #course 从第5个数据开始利用，前三个用了，第4个空着用来隔离。TSS0（进程0的tss段）数据写到了第5个数据段
...
/* Clear NT, so that we won't have troubles with that later on */
	...
	set_intr_gate(0x20,&timer_interrupt);	//tsz: #course 终端
