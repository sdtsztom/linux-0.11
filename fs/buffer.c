...
#define _hashfn(dev,block) (((unsigned)(dev^block))%NR_HASH)	// tsz: #course 用设备号和块号做hash；#ques 为什么要用按位异或
...
#define BADNESS(bh) (((bh)->b_dirt<<1)+(bh)->b_lock)	// tsz: #course 增加dirt的权重,先dirt后同步，所以先选同步的块，等的时间更短
//// 取高速缓冲中指定的缓冲块
...
/* and that it's unused (b_count=0), unlocked (b_lock=0), and clean */
    ...
    // 位置处。并最终返回缓冲头指针。
	remove_from_queues(bh);	// tsz: #course #TODO 自己回去看，回去算下挂在哪个项上了
	bh->b_dev=dev;	// tsz: #course 在这里赋值，然后再ll_rw_block()中用到
	bh->b_blocknr=block;
...
struct buffer_head * bread(int dev,int block)
{
    ...
    // 可以直接使用，则返回。
	if (!(bh=getblk(dev,block)))	// tsz: #course #impo #EMP 核心实现：缓冲区用**复用**来为多线程提高读写效率;让块在缓冲区待得尽量长
...
void buffer_init(long buffer_end)
{
	...
	if (buffer_end == 1<<20)
		b = (void *) (640*1024);	// tsz: #personal 刚好是ldt0中数据、代码描述符的段限长,同时也是BIOS区域的开始;对BIOS区域的保留处理1
	else
		b = (void *) buffer_end;
	...
	//tsz: #think 共有多少对：#answ 在1MB内存下，共有(640-64-196)*1024/(1024+34)=367块；16MB下，同理有(3*1024+640-64-196)*1024/(1024+34)=3341块（应该是错的，因为end是编译时确定的，不能只考虑代码区大小）
	while ( (b -= BLOCK_SIZE) >= ((void *) (h+1)) ) {	// tsz: #book 1k/block
		...
		h++;                                // h指向下一新缓冲头位置
		NR_BUFFERS++;                       // 缓冲区块数累加
		if (b == (void *) 0x100000)         // 若b递减到等于1MB，则跳过384KB	// tsz: #personal 跳过了bios和显存区段;所以bios区域一直有被care着保留不动;说明了bios的缓存区在系统启动以后还是需要的;对BIOS区域的保留处理2
