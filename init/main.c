 ...
 */
	...
	memory_end = (1<<20) + (EXT_MEM_K<<10);     // 内存大小=1Mb + 扩展内存(k)*1024 byte	// tsz: #personal 既然只能到16MB，那为什么这里EXT_MEM_K还这么大
	memory_end &= 0xfffff000;                   // 忽略不到4kb(1页)的内存数
	if (memory_end > 16*1024*1024)              // 内存超过16Mb，则按16Mb计	// tsz: #personal 内存管理大小的限制1
