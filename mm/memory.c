...
int copy_page_tables(unsigned long from,unsigned long to,long size)	// tsz: #personal 对于进程1来说,from是0，to是其对应的64M内存空间的起始地址，size是父进程数据段限长，对于进程0来说也就是640K;复制了父进程的页目录表项和页表说明重用了父进程的页，为什么如此下面会讲到
{
	...
	from_dir = (unsigned long *) ((from>>20) & 0xffc); /* _pg_dir = 0 */	// tsz:#personal 根本目的是从地址转化到其对应的管理这个地址的页目录表项地址 #course >>20是以MB为单位，和ffc与之后，是4MB的倍数;原理：假设页目录表项N，其起始地址在4B*(N-1)处，这个页目录表项管理的地址在4MB*(N-1)-4MB*N(因为一个页表管理4MB) => 页目录表项的起始地址与管理的地址差1兆倍数。&0xffc是当地址不整齐时，让算出来页目录表项地址与4B对齐(与先与4MB对齐之后再算页目录表项地址目的一致)
	to_dir = (unsigned long *) ((to>>20) & 0xffc);	// tsz: #personal #note #impo 从子进程的地址得到管理其地址空间的页目录表项的地址，进程1是从64mb开始，对应的第17个页目录表项，即页目录表项16;一个进程最多占用16个页目录表项
	...
	for( ; size-->0 ; from_dir++,to_dir++) {	// tsz: #personal 大循环，遍历页目录表项（也相当于在遍历页表），每个循环设置一个新进程的页目录表项，同时也复制一个页表
		...
		from_page_table = (unsigned long *) (0xfffff000 & *from_dir);	// tsz: #personal 这是要复制的页表的指针，从对应的页目录表项中得到页目录表项，被清除属性后就是指向页表的指针，;#course 清零最后12bit，他就从页目录表项中得到了指向页表的指针
		if (!(to_page_table = (unsigned long *) get_free_page()))	// tsz: #course 注意这个get_free_page是从内核中获得空页，用这个页去创建复制的页表；说明新进程页表在内核空间里面(倘若在用户空间，可以轻易修改task_struct和页表指向内容)(注意新进程的页目录表在自己空间里，见to_dir)
		...
		*to_dir = ((unsigned long) to_page_table) | 7;	// tsz: #course  7:存在/可读写/u;页目录表项在用户程序空间，而页表则在内核空间;#personal 设置新进程的页目录表项
        ...
        // 到目录页表中。
		for ( ; nr-- > 0 ; from_page_table++,to_page_table++) {	// tsz: #course 直接用父进程内容的原因：1.服务器中有很多线程执行的是相同的代码；2. 依托父进程的代码来创建自己的进程内容；#personal 小循环，每个循环遍历一个页表项（也同时在遍历一个页），复制一个页的页表项
			this_page = *from_page_table;	// tsz: #personal 这里得到了页表项，指向一个页
			if (!(1 & this_page))	// tsz: #course 检查是否存在
				continue;
			this_page &= ~2;	// tsz: #course 给第二位清零，设置只读，因为共享了父进程的空间；当真要写的时候进行写时复制;#impo 注意区别，页目录表项和页表都是进程私有的，但拥有的页面是共同的;子进程和父进程的页表指向的页地址都一样;子进程对自己的页目录表项拥有7权限，即可修改页表内容，但是其页表项为只读权限，表示对页只读
